...
async function initialSetup() {
    try {
        ...
        for (const saEmail of superAdminEmails) {
            const superAdminPasswordHash = await bcrypt.hash('Jevish2026!', 12);
            ...
            if (!created) {
                ...
                logger.info(`Super Admin password & role verified in setup: ${saEmail}`);
            } else {
                logger.info(`Super Admin created in setup: ${saEmail}`);
            }
        }
        ...
        logger.info('Commercial SaaS "Jevish Alpha" working data initialized.');
