...
router.post('/quick-onboard', async (req, res) => {
    try {
        ...
        let tenantId = user?.tenantId || req.body?.tenantId || (req.headers['x-tenant-id'] as string);
        ...
        const onboardToken = crypto.randomBytes(16).toString('hex');
        ...
        logger.info('Automated router onboarding initiated', {
        ...
        logger.error('Quick onboarding failed', { error: error.message, stack: error.stack });
...
router.get('/:id/discovery', async (req, res) => {
    try {
        ...
        logger.error('Failed to discover devices', { error: error.message });
