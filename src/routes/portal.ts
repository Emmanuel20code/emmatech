...
import { Package, Payment, Tenant, Router as RouterModel, PlatformSetting, MarketingCoupon, Voucher, Session } from '../models';
...
const resolveTenant = async (idOrSubdomain: string) => {
...
router.get('/:tenantId/ads', async (req: any, res) => {
    try {
        ...
        logger.error('Captive portal ad fetch error, returning fallback', { error: error.message });
        res.json([]);
    }
});
...
router.get('/:tenantId/packages', async (req: any, res) => {
    try {
        ...
        logger.error('Error fetching portal packages', { tenantId: req.params.tenantId, error: error.message });
        res.status(500).json({ error: error.message });
    }
});

// 2. Initiate M-Pesa STK Push Payment (Hotspot or ISP)
router.post('/:tenantId/pay', async (req: any, res: any) => {
    try {
        ...
        if (validRouterId === 'unknown' || !validRouterId) {
            const firstRouter = await RouterModel.findOne({ where: { tenantId: actualTenantId } });
        ...
        try {
            logger.info('[Portal Pay] Initiating STK push', {
                tenantId: actualTenantId,
            ...
            logger.error('[Portal Pay] STK Push initiation failed', { error: stkErr.message });
        ...
        logger.error('[Portal Pay] Unexpected error', { error: e.message });
...
router.post('/:tenantId/verify-receipt', async (req: any, res: any) => {
    try {
        ...
        const normalizedPayment = {
            ...
            paymentChannel: 'MPESA_PAYBILL' as const,
        ...
        logger.error('[Portal] Verify receipt failed', { error: err.message });
...
router.get('/payment-status/:id', async (req, res) => {
    try {
        ...
        const result: any = {
            ...
            package: (payment as any).package || null
        };

        if (payment.status === 'SUCCESS') {
            if ((payment as any).session) {
                result.credentials = {
                    username: (payment as any).session.mikrotikUsername,
                    password: (payment as any).session.mikrotikPassword
                };
            }
        ...
        logger.error('[Portal] Payment status polling error', { paymentId: req.params.id, error: error.message });
