...
router.get('/integrations', async (req: Request, res: Response): Promise<void> => {
    try {
        const tenantId = (req as any).tenantId || (req as any).user?.tenantId;
        const isSuperAdmin = (req as any).user?.role === 'SUPER_ADMIN' || (req as any).user?.role === 'PLATFORM_OWNER';
        const tenant = tenantId ? await Tenant.findByPk(tenantId) : null;
        const routerCount = tenantId ? await RouterModel.count({ where: { tenantId, isOnline: true } }) : 0;
...
const maskCredential = (val: string | null): string => {
    if (!val) return '';
    try {
...
router.get('/integrations/mpesa', async (req: Request, res: Response): Promise<void> => {
    try {
        const isSuperAdmin = (req as any).user?.role === 'SUPER_ADMIN' || (req as any).user?.role === 'PLATFORM_OWNER';
        const tenantId = (req as any).tenantId || (req as any).user?.tenantId;
...
router.put('/integrations/mpesa', async (req: Request, res: Response): Promise<void> => {
    try {
        const isSuperAdmin = (req as any).user?.role === 'SUPER_ADMIN' || (req as any).user?.role === 'PLATFORM_OWNER';
        const tenantId = (req as any).tenantId || (req as any).user?.tenantId;
        ...
        if (isSuperAdmin) {
            ...
            if (mpesaShortcode !== undefined) await savePlatformSetting('SUPERADMIN_MPESA_SHORTCODE', mpesaShortcode || '');
            if (mpesaTillNumber !== undefined) await savePlatformSetting('SUPERADMIN_MPESA_TILL', mpesaTillNumber || '');
            if (mpesaPaybillNumber !== undefined) await savePlatformSetting('SUPERADMIN_MPESA_PAYBILL', mpesaPaybillNumber || '');
            if (mpesaEnvironment !== undefined) await savePlatformSetting('SUPERADMIN_MPESA_ENV', mpesaEnvironment || 'sandbox');
            if (mpesaInitiatorName !== undefined) await savePlatformSetting('SUPERADMIN_MPESA_INITIATOR_NAME', mpesaInitiatorName || '');
            ...
            if (tenantId) {
                const tenant = await Tenant.findByPk(tenantId);
                if (tenant) {
                    if (mpesaShortcode !== undefined) tenant.mpesaShortcode = mpesaShortcode || null;
                    if (mpesaTillNumber !== undefined) tenant.mpesaTillNumber = mpesaTillNumber || null;
                    if (mpesaPaybillNumber !== undefined) tenant.mpesaPaybillNumber = mpesaPaybillNumber || null;
                    if (mpesaEnvironment !== undefined) tenant.mpesaEnvironment = mpesaEnvironment || 'sandbox';
                    if (mpesaInitiatorName !== undefined) tenant.mpesaInitiatorName = mpesaInitiatorName || null;
...
router.post('/integrations/mpesa/test', async (req: Request, res: Response): Promise<void> => {
    try {
        const isSuperAdmin = (req as any).user?.role === 'SUPER_ADMIN' || (req as any).user?.role === 'PLATFORM_OWNER';
        const tenantId = (req as any).tenantId || (req as any).user?.tenantId;
