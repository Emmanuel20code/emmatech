...
router.get('/', async (req, res) => {
    try {
        ...
        const bindings = await models_1.DeviceBinding.findAll({
            where: { tenantId },
            include: [
                { model: models_1.Subscriber, attributes: ['id', 'name', 'username', 'phoneNumber', 'email'] },
                { model: models_1.Router, attributes: ['id', 'name', 'host', 'port', 'isOnline'] }
            ],
        ...
        logger_1.default.error('Error fetching device bindings', { error: e.message });
...
router.get('/stats', async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const bindings = await models_1.DeviceBinding.findAll({ where: { tenantId } });
        const stats = {
...
router.get('/discover', async (req, res) => {
    try {
        ...
        const routers = await models_1.Router.findAll({ where: routerQuery });
        if (routers.length === 0) {
            return res.json({ devices: [], total: 0, scannedRouters: 0, message: 'No routers configured' });
        }
        const existingBindings = await models_1.DeviceBinding.findAll({ where: { tenantId } });
        const boundMacMap = new Map();
        ...
        for (const r of routers) {
            ...
            try {
                await mikrotik_service_1.MikroTikService.executeWithRetry(async () => {
                    const client = await mikrotik_service_1.MikroTikService.getConnection(r);
                ...
                logger_1.default.debug('Router scan offline/fallback to database records', { routerId: r.id, error: err.message });
            }
            ...
            if (!liveHostsFound || discoveredMap.size === 0) {
                try {
                    const recentSessions = await models_1.Session.findAll({
                        where: {
                            tenantId,
                            routerId: r.id,
                            macAddress: { [sequelize_1.Op.ne]: null }
                        },
                        order: [['createdAt', 'DESC']],
                        limit: 25
                    });
                    for (const s of recentSessions) {
                        ...
                        if (!mac || !isValidMac(mac) || discoveredMap.has(mac))
                            continue;
                        const existing = boundMacMap.get(mac);
        ...
        const devices = Array.from(discoveredMap.values()).sort((a, b) => {
            ...
            if (a.isBound !== b.isBound)
                return a.isBound ? 1 : -1;
            if (a.deviceType === 'TV' && b.deviceType !== 'TV')
                return -1;
            if (b.deviceType === 'TV' && a.deviceType !== 'TV')
                return 1;
            return 0;
        });
        res.json({
        ...
        logger_1.default.error('Error discovering devices', { error: e.message });
        res.status(500).json({ error: e.message });
    }
});
/**
...
router.post('/', async (req, res) => {
    try {
        ...
        const routerData = await models_1.Router.findOne({ where: { id: routerId, tenantId } });
        if (!routerData) {
        ...
        try {
            await mikrotik_service_1.MikroTikService.executeWithRetry(async () => {
                const client = await mikrotik_service_1.MikroTikService.getConnection(routerData);
                const api = await client.connect();
            ...
            logger_1.default.warn('Failed to sync binding to MikroTik hardware (saved to database)', { routerId, mac: normalizedMac, error: e.message });
        }
        // Check if binding exists in database
        let binding = await models_1.DeviceBinding.findOne({
        ...
        if (binding) {
            ...
            // Create new record
            binding = await models_1.DeviceBinding.create({
                tenantId,
                routerId,
        ...
        const completeBinding = await models_1.DeviceBinding.findByPk(binding.id, {
            include: [
                { model: models_1.Subscriber, attributes: ['id', 'name', 'username', 'phoneNumber'] },
                { model: models_1.Router, attributes: ['id', 'name', 'host', 'port', 'isOnline'] }
            ]
        });
        res.json({
        ...
        logger_1.default.error('Error creating device binding', { error: e.message });
        res.status(500).json({ error: e.message });
    }
});
/**
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const binding = await models_1.DeviceBinding.findOne({ where: { id: req.params.id, tenantId } });
        if (!binding)
        ...
        await binding.save();
        // Update in MikroTik
        const routerData = await models_1.Router.findOne({ where: { id: binding.routerId, tenantId } });
        if (routerData) {
            try {
                await mikrotik_service_1.MikroTikService.executeWithRetry(async () => {
                    const client = await mikrotik_service_1.MikroTikService.getConnection(routerData);
                ...
                logger_1.default.warn('Failed to update binding in MikroTik', { error: e.message });
            }
        }
        const updated = await models_1.DeviceBinding.findByPk(binding.id, {
            include: [
                { model: models_1.Subscriber, attributes: ['id', 'name', 'username', 'phoneNumber'] },
                { model: models_1.Router, attributes: ['id', 'name'] }
            ]
        });
    ...
    try {
        ...
        if (routerData) {
            try {
                ...
                logger_1.default.warn('Failed to sync suspension status to MikroTik', { error: e.message });
            }
        }
        res.json({
            success: true,
    ...
    try {
        ...
        if (routerData) {
            try {
                ...
                logger_1.default.warn('Failed to remove Mikrotik IP binding from router', { error: e.message });
            }
        }
