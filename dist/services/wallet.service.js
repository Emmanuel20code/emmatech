...
class WalletService {
    ...
    static async initializeTenantWallet(tenantId, transaction) {
        try {
            ...
            await audit_service_1.AuditService.log('WALLET_INITIALIZED', `Tenant wallet initialized: ${tenantId}`, tenantId, undefined);
            return wallet;
        }
        catch (error) {
            logger_1.default.error('Failed to initialize tenant wallet', { error: error instanceof Error ? error.message : String(error), tenantId });
    ...
    static async getWalletBalanceByOwner(ownerId, ownerType = 'TENANT') {
        const wallet = await models_1.Wallet.findOne({ where: { ownerId, ownerType } });
        ...
        try {
            ...
            totalEarnings = await models_1.WalletTransaction.sum('amount', {
                where: { walletId: wallet.id, transactionType: 'CREDIT' }
            }) || 0;
            totalFees = await Payment.sum('platformFee', {
                where: { tenantId: ownerId, status: 'SUCCESS' }
            }) || 0;
        }
        catch (err) {
            logger_1.default.error('Error calculating wallet analytics', { error: err instanceof Error ? err.message : String(err) });
        }
        return {
    ...
    static async getWalletBalance(walletId) {
        const wallet = await models_1.Wallet.findByPk(walletId);
        if (!wallet)
    ...
    static async processPayment(payment, externalTransaction) {
        const transaction = externalTransaction || await models_1.sequelize.transaction();
        try {
            const tenant = await models_1.Tenant.findByPk(payment.tenantId);
            if (!tenant)
            ...
            const walletTransaction = await models_1.WalletTransaction.create({
                walletId: wallet.id,
            ...
            if (commissionAmount > 0) {
                await models_1.PlatformTransaction.create({
                    type: 'COMMISSION',
            ...
            await audit_service_1.AuditService.log('PAYMENT_AGGREGATOR_SPLIT', `Split processed for ${payment.id}: Tenant(${netAmount}) Platform(${platformFeeAmount})`, payment.tenantId, payment.subscriberId || undefined);
            return walletTransaction;
        }
        catch (error) {
            if (!externalTransaction)
                await transaction.rollback();
            logger_1.default.error('Aggregator split processing failed', { error: error instanceof Error ? error.message : String(error), paymentId: payment.id });
    ...
    static async clearAllMaturedPendingBalances() {
        ...
        const maturedTransactions = await models_1.WalletTransaction.findAll({
            where: {
        ...
        for (const tx of maturedTransactions) {
            try {
                ...
                logger_1.default.error('Failed to clear matured pending balance', { transactionId: tx.id, error: error instanceof Error ? error.message : String(error) });
            }
        }
        if (clearedCount > 0) {
            logger_1.default.info(`Cleared ${clearedCount} matured pending transactions`);
        }
        return clearedCount;
    }
    /**
    ...
    static async clearPendingBalance(walletId, amount) {
        ...
        try {
            ...
            if (!wallet)
                throw new Error('Wallet not found');
            if (BigInt(wallet.pendingBalance) < BigInt(amount))
    ...
    static async updatePlatformWallet(amount, transactionType, _referenceId, transaction) {
        let platformWallet = await models_1.PlatformWallet.findOne();
        if (!platformWallet) {
            platformWallet = await models_1.PlatformWallet.create({ balance: 0, pendingBalance: 0, currency: 'KES' }, { transaction });
        }
    ...
    static async createSettlement(tenantId, amount, method, userId) {
        ...
        try {
            ...
            if (BigInt(wallet.settledBalance) < BigInt(amount))
                throw new Error('Insufficient settled balance for withdrawal');
            await wallet.update({
                settledBalance: BigInt(wallet.settledBalance) - BigInt(amount),
            ...
            await transaction.commit();
            await audit_service_1.AuditService.log('SETTLEMENT_REQUESTED', `Settlement requested: ${amount} via ${method}`, tenantId, userId || undefined);
            return settlement;
        }
        catch (error) {
            await transaction.rollback();
            logger_1.default.error('Failed to create settlement', { error: error instanceof Error ? error.message : String(error), tenantId, amount });
    ...
    static async processSettlement(settlementId, userId) {
        const transaction = await models_1.sequelize.transaction();
        try {
            const settlement = await models_1.Settlement.findByPk(settlementId, { transaction });
            if (!settlement)
                throw new Error('Settlement not found');
            if (settlement.status !== 'PENDING')
                throw new Error('Settlement already processed');
            const tenant = await models_1.Tenant.findByPk(settlement.tenantId);
            if (!tenant)
                throw new Error('Tenant not found');
            const wallet = await models_1.Wallet.findOne({
            ...
            const walletTransaction = await models_1.WalletTransaction.findByPk(settlement.walletTransactionId || undefined);
            ...
            await transaction.commit();
            await audit_service_1.AuditService.log('SETTLEMENT_PROCESSED', `Settlement ${settlementId} processed: ${settlement.amount} paid via ${settlement.method}`, settlement.tenantId, userId || undefined);
            return settlement;
        }
        catch (error) {
            await transaction.rollback();
            logger_1.default.error('Failed to process settlement', { error: error instanceof Error ? error.message : String(error), settlementId });
            throw new Error(error instanceof Error ? error.message : String(error));
        }
    }
    /**
    ...
    static async reverseTransaction(transactionId, reason, userId) {
        const transaction = await models_1.sequelize.transaction();
        try {
            const walletTransaction = await models_1.WalletTransaction.findByPk(transactionId, { transaction });
            if (!walletTransaction)
                throw new Error('Transaction not found');
            if (walletTransaction.status === 'REVERSED')
                throw new Error('Transaction already reversed');
            const wallet = await models_1.Wallet.findByPk(walletTransaction.walletId, { transaction });
            if (!wallet)
                throw new Error('Wallet not found');
            ...
            const reversalTransaction = await models_1.WalletTransaction.create({
                walletId: wallet.id,
            ...
            if (walletTransaction.referenceType === 'PAYMENT') {
                const payment = await models_1.Payment.findByPk(walletTransaction.referenceId || undefined);
                if (payment) {
                    await payment.update({
                        status: 'REVERSED'
                    }, { transaction });
            ...
            if (walletTransaction.referenceType === 'SETTLEMENT') {
                const settlement = await models_1.Settlement.findByPk(walletTransaction.referenceId || undefined);
                if (settlement) {
                    await settlement.update({
                        status: 'REVERSED'
                    }, { transaction });
            ...
            await transaction.commit();
            await audit_service_1.AuditService.log('TRANSACTION_REVERSED', `Transaction ${transactionId} reversed: ${reason}`, walletTransaction.tenantId, userId || undefined);
            return reversalTransaction;
        }
        catch (error) {
            await transaction.rollback();
            logger_1.default.error('Failed to reverse transaction', { error: error instanceof Error ? error.message : String(error), transactionId });
            throw new Error(error instanceof Error ? error.message : String(error));
        }
    }
    /**
    ...
    static async getWalletTransactionsByOwner(ownerId, ownerType = 'TENANT', limit = 50, offset = 0) {
        ...
        if (!wallet)
            return [];
        return models_1.WalletTransaction.findAll({
    ...
    static async manualAdjustment(walletId, amount, reason, userId) {
        const transaction = await models_1.sequelize.transaction();
        try {
            const wallet = await models_1.Wallet.findByPk(walletId, { transaction });
            if (!wallet)
                throw new Error('Wallet not found');
            ...
            await transaction.commit();
            await audit_service_1.AuditService.log('WALLET_ADJUSTMENT', `Manual adjustment: ${amount} - ${reason}`, wallet.tenantId, userId || undefined);
            return walletTransaction;
        }
        catch (error) {
            await transaction.rollback();
            logger_1.default.error('Failed to make manual adjustment', { error: error instanceof Error ? error.message : String(error), walletId });
            throw new Error(error instanceof Error ? error.message : String(error));
        }
    }
    /**
    ...
    static async billSms(tenantId, smsLogId) {
        ...
        try {
            ...
            await transaction.rollback();
            logger_1.default.error('Failed to bill SMS usage', { error: error instanceof Error ? error.message : String(error), tenantId });
        }
    }
    /**
    ...
    static async billSubscription(tenantId) {
        ...
        try {
            ...
            await transaction.rollback();
            logger_1.default.error('Failed to bill subscription', { error: error instanceof Error ? error.message : String(error), tenantId });
        }
    }
    /**
    ...
    static async getPlatformWalletBalance() {
        const platformWallet = await models_1.PlatformWallet.findOne() || { balance: BigInt(0), pendingBalance: BigInt(0), currency: 'KES' };
        return {
    ...
    static async getTransactionTrace(transactionId, tenantId) {
        const tx = await models_1.WalletTransaction.findOne({
        ...
        if (tx.referenceType === 'PAYMENT' && tx.referenceId) {
            const payment = await models_1.Payment.findByPk(tx.referenceId);
            if (payment) {
                sourceData = {
            ...
            const settlement = await models_1.Settlement.findByPk(tx.referenceId);
            if (settlement) {
                sourceData = {
                    type: 'SETTLEMENT',
        ...
        const auditLogs = await models_1.AuditLog.findAll({
            where: {
                tenantId,
                details: { [sequelize_1.Op.like]: `%${transactionId}%` }
            },
            order: [['createdAt', 'DESC']]
        });
        return {
