...
class MpesaService {
    ...
    static getCallbackUrl(path, req) {
        ...
        if (!callbackBase || callbackBase.includes('localhost') || callbackBase.includes('127.0.0.1')) {
            callbackBase = process.env.MPESA_CALLBACK_BASE_URL || env_1.config.app.url || 'https://ais-dev-rrrw5fw3ibuhz3wldvwzie-342377543254.europe-west3.run.app';
        }
        // Ensure https
     ...
     */
    static async initiateStkPush(phoneNumber, amount, tenantId, userId = 'guest', packageId = '1', req) {
    ...
    static async checkTransactionStatus(checkoutRequestID, targetTenantId) {
        try {
            ...
            if (errData?.errorCode === '500.001.1001' || (typeof errMsg === 'string' && errMsg.includes('does not Exist'))) {
                logger_1.default.debug('[MpesaService] STK Query: Transaction not found yet or pending', { checkoutRequestID });
            }
            else {
