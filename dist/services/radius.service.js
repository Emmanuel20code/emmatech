...
class RadiusService {
    ...
    static async syncSubscriberAttributes(subscriberId, tenantId) {
        const sub = await models_1.Subscriber.findOne({
            where: { id: subscriberId, tenantId },
            include: [{ model: models_1.Package, as: 'package' }]
        });
        if (!sub) {
        ...
        await models_1.RadCheck.destroy({ where: { username, tenantId } });
        await models_1.RadReply.destroy({ where: { username, tenantId } });
        await models_1.RadUserGroup.destroy({ where: { username, tenantId } });
        // Add radcheck authentication entry
        await models_1.RadCheck.create({
            username,
        ...
        if (pkg) {
            ...
            await models_1.RadReply.create({
                username,
            ...
            // Assign User Group
            await models_1.RadUserGroup.create({
                username,
    ...
    static async authenticateSubscriber(params) {
        ...
        if (voucherCode || serviceType === 'Voucher') {
            ...
            const voucher = await models_1.Voucher.findOne({ where: { code: codeToVerify, tenantId } });
        ...
        // 2. MAC Authentication
            ...
            subscriberRecord = await models_1.Subscriber.findOne({
        ...
        // 3. Username / Password (PPPoE & Hotspot)
            ...
            const radCheckRecord = await models_1.RadCheck.findOne({
        ...
        await models_1.RadPostAuth.create({
            username,
            pass: password || '***',
        ...
        const replyAttrs = await models_1.RadReply.findAll({ where: { username, tenantId } });
        const attributesMap = {};
    ...
    static async processAccounting(packet) {
        ...
        const now = new Date();
        let acctRecord = await models_1.RadAcct.findOne({ where: { acctuniqueid: uniqueId } });
        if (packet.acctstatusType === 'Start') {
            if (!acctRecord) {
                acctRecord = await models_1.RadAcct.create({
    ...
    static async sendDisconnectMessage(params) {
        ...
        return new Promise((resolve) => {
            ...
            const authenticator = crypto_1.default.randomBytes(16);
            ...
            const md5Hasher = crypto_1.default.createHash('md5');
            ...
            client.send(packet, port, nasIp, (err) => {
                client.close();
                if (err) {
                    logger_1.default.error(`Failed to send RADIUS Disconnect packet to ${nasIp}:${port}: ${err.message}`);
                    resolve({ success: false, error: err.message });
                }
                else {
                    logger_1.default.info(`RADIUS Disconnect-Request (PoD) sent to ${nasIp}:${port} for user ${username}`);
    ...
    static async sendCoAMessage(params) {
        ...
        return new Promise((resolve) => {
            ...
            client.send(packet, port, nasIp, (err) => {
                client.close();
                if (err) {
                    logger_1.default.error(`Failed to send RADIUS CoA packet to ${nasIp}:${port}: ${err.message}`);
                    resolve({ success: false, error: err.message });
                }
                else {
                    logger_1.default.info(`RADIUS CoA-Request sent to ${nasIp}:${port} for user ${username} (Rate Limit: ${rateLimit})`);
    ...
    static async getRadiusOverview(tenantId) {
        ...
        const activeSessions = await models_1.RadAcct.count({
            where: { ...whereTenant, acctstoptime: null }
        });
        const totalNas = await models_1.Nas.count({ where: whereTenant });
        const authRequestsCount = await models_1.RadPostAuth.count({ where: whereTenant });
        const failedAuthsCount = await models_1.RadPostAuth.count({
            where: { ...whereTenant, reply: 'Access-Reject' }
        });
        const recentSessions = await models_1.RadAcct.findAll({
            where: whereTenant,
            order: [['acctstarttime', 'DESC']],
            limit: 15
        });
        const nasList = await models_1.Nas.findAll({ where: whereTenant });
        return {
            stats: {
                activeSessions,
                totalNas,
