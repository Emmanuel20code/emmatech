...
class BrandingService {
    ...
    static async getPlatformBranding() {
        ...
        try {
            const setting = await models_1.PlatformSetting.findByPk('platform_branding');
            if (setting) {
                return { ...defaultBranding, ...JSON.parse(setting.value) };
            }
        }
        catch (error) {
            logger_1.default.error('Failed to fetch platform branding from settings', { error });
        }
    ...
    static async updatePlatformBranding(data) {
        try {
            ...
            const [setting] = await models_1.PlatformSetting.upsert({
                key: 'platform_branding',
                value: JSON.stringify(updated)
            });
            logger_1.default.info('Platform branding updated by Super Admin');
            return updated;
        }
        catch (error) {
    ...
    static async getTenantCaptivePortalBranding(tenantIdOrIdentifier) {
        ...
        let tenant = await models_1.Tenant.findOne({
            where: {
                [sequelize_1.Op.or]: [
        ...
        if (tenant) {
            brandingRecord = await models_1.TenantCaptivePortalBranding.findOne({
            ...
            brandingRecord = await models_1.TenantCaptivePortalBranding.findOne({
                ...
                include: [{ model: models_1.Tenant, as: 'tenant' }]
            });
    ...
    static async updateTenantCaptivePortalBranding(tenantId, data) {
        try {
            ...
            const existing = await models_1.TenantCaptivePortalBranding.findOne({
                where: { tenantId }
            });
            ...
            if (existing) {
                ...
                await models_1.TenantCaptivePortalBranding.create({ ...preparedData, tenantId });
            }
            ...
            logger_1.default.error('Failed to update tenant branding in service', { tenantId, error: error.message, stack: error.stack });
    ...
    static async resetTenantCaptivePortalBranding(tenantId) {
        await models_1.TenantCaptivePortalBranding.destroy({
            where: { tenantId }
        });
        await models_1.AuditLog.create({
